...
$BackupRoot = Join-Path $ProjectRoot ("backups\arthur-v3.6.1-" + (Get-Date -Format "yyyyMMdd-HHmmss") + "-" + [guid]::NewGuid().ToString("N").Substring(0, 6))
...
try {
    ...
    Write-Host "Arthur v3.6.1 installed. Files verified: $($Files.Count). Backup: $BackupRoot"
